...
class NcField(Field):
    ...
    def _get_value_from_source_(self, data, variable_name):
        ...
        axis_reshape = {}
        # tdk: reference duplicate variables
        if self.temporal is not None:
            axis_slc[self.temporal.name] = self.temporal._src_idx
            axis_reshape[self.temporal.axis] = self.temporal._src_idx
        try:
            axis_slc[self.spatial.grid.row.name] = self.spatial.grid.row._src_idx
            axis_slc[self.spatial.grid.col.name] = self.spatial.grid.col._src_idx
            axis_reshape[self.spatial.grid.row.axis] = self.spatial.grid.row._src_idx
            axis_reshape[self.spatial.grid.col.axis] = self.spatial.grid.col._src_idx
        # If grid and row are not present on the grid object. The source indices are attached to the grid object.
        except AttributeError:
            axis_slc[self.spatial.grid.name_row] = self.spatial.grid._src_idx['row']
            axis_slc[self.spatial.grid.name_col] = self.spatial.grid._src_idx['col']
            axis_reshape['Y'] = self.spatial.grid._src_idx['row']
            axis_reshape['X'] = self.spatial.grid._src_idx['col']
        if self.realization is not None:
            axis_slc[self.realization.name] = self.realization._src_idx
            axis_reshape[self.realization.axis] = self.realization._src_idx
        if self.level is not None:
            axis_slc[self.level.name] = self.level._src_idx
            axis_reshape[self.level.axis] = self.level._src_idx
